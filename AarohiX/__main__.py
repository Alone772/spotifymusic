...
async def init():
    ...
    await Dil.start()
    try:
        await Dil.stream_call("https://envs.sh/s7K.mp4")
    ...
    LOGGER("AarohiX").info(
        "ᴍᴜsɪᴄ ʙᴏᴛ sᴛᴀʀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ, ɴᴏᴡ ɢɪʙ ʏᴏᴜʀ ɢɪʀʟғʀɪᴇɴᴅ ᴄʜᴜᴛ ɪɴ @tmm_heroku_world"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("AarohiX").info("ᴍᴀᴀ ᴄʜᴜᴅᴀ ᴍᴀɪɴ ʙᴏᴛ ʙᴀɴᴅ ᴋᴀʀ ʀʜᴀ sᴘᴏᴛɪғʏ ꭙ ᴍᴜsɪᴄ Bᴏᴛ...")
